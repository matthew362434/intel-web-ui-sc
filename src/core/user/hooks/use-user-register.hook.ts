...
interface UseUserRegisterMutation {
    registerUser: UseMutationResult<unknown, AxiosError, UserRegistrationDTO>;
}

export const useUserRegister = (): UseUserRegisterMutation => {
    const { userService } = useUserService();
    const { addNotification } = useNotification();
    const registerUser = useMutation<unknown, AxiosError, UserRegistrationDTO>(
        async (body: UserRegistrationDTO) => {
