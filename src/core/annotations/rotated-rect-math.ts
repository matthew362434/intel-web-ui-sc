...
export const transformPointInRotatedRectToScreenSpace = (position: Vec2.Vec2, shape: RotatedRect): Vec2.Vec2 => {
    ...
    const rotated = Vec2.rotateDeg(temp, shape.angle);
...
export const calculateSizeAndPositionBasedOfCornerAnchor = (
    ...
    const size = Vec2.rotateDeg(Vec2.sub(oppositeCorner, position), -shape.angle);
    const sign = Vec2.sign(Vec2.rotateDeg(Vec2.sub(oppositeCorner, corner), -shape.angle));
    const dimensions = Vec2.mul(sign, size);
    const constrainedDimensions = {
        x: Math.max(dimensions.x, gap),
        y: Math.max(dimensions.y, gap),
    };
    const constrainedPosition = Vec2.sub(
        oppositeCorner,
        Vec2.rotateDeg(Vec2.mul(constrainedDimensions, sign), shape.angle)
    );
...
export const rotatedRectCorners = (shape: RotatedRect): Vec2.Vec2[] => {
    ...
    return [
        Vec2.add(Vec2.rotateDeg(Vec2.mul(halfSize, { x: -1, y: -1 }), angle), position),
        Vec2.add(Vec2.rotateDeg(Vec2.mul(halfSize, { x: 1, y: -1 }), angle), position),
        Vec2.add(Vec2.rotateDeg(Vec2.mul(halfSize, { x: 1, y: 1 }), angle), position),
        Vec2.add(Vec2.rotateDeg(Vec2.mul(halfSize, { x: -1, y: 1 }), angle), position),
    ];
...
export const highestCorner = (shape: RotatedRect): Vec2.Vec2 => {
