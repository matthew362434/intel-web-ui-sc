...
import {
    ...
    ConfigurableParametersGroupBasicParamsDTO,
} from '../../../core/configurable-parameters/dtos';
import { HPOProps } from '../../../pages/project-details/components/project-models/train-model-dialog/use-training-state-value';

interface ConfigParametersId {
    id: string;
    autoHPOState: ConfigurableParametersGroupBasicParamsDTO['auto_hpo_state'];
}

type HPOOmitTypes = 'auto_hpo_state' | 'auto_hpo_value';
...
export interface SelectableGroupParams<T extends string | number, K extends ConfigGroupParametersDataTypesDTO>
    extends ConfigParametersId,
        Omit<SelectableGroupParamsDTO<T, K>, 'template_type' | 'data_type' | 'default_value' | HPOOmitTypes> {
    ...
    autoHPOValue: SelectableGroupParamsDTO<T, K>['auto_hpo_value'];
}
...
export interface NumberGroupParams
    extends ConfigParametersId,
        Omit<
            NumberGroupParamsDTO,
            'template_type' | 'data_type' | 'min_value' | 'max_value' | 'default_value' | HPOOmitTypes
        > {
    ...
    autoHPOValue: NumberGroupParamsDTO['auto_hpo_value'];
}

export interface BooleanGroupParams
    extends ConfigParametersId,
        Omit<BooleanGroupParamsDTO, 'template_type' | 'data_type' | 'default_value' | HPOOmitTypes> {
    ...
    autoHPOValue: BooleanGroupParamsDTO['auto_hpo_value'];
}
...
export type ConfigurableParametersProps =
    | (ConfigurableParametersSingle & { hpo: HPOProps })
    | ConfigurableParametersSingleReadOnly
    | ConfigurableParametersMany;

export interface ConfigParameterItemProp extends Partial<Pick<ConfigurableParametersSingle, 'updateParameter'>> {
    isHPO: boolean;
}
