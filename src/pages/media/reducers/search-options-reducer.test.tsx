...
import { AdvancedFilterOptions, SearchOptionsActionsType, SearchOptionsRule } from '../media-filter.interface';
import { SearchOptionReducer } from './search-options-reducer';

const getMockRule = (value = 'test1', id: string, isUnremovable = false): SearchOptionsRule => ({
    id,
    field: '',
    operator: '',
    value,
...
describe('SearchOptionReducer', () => {
    ...
    it('remove all but hidden rules', () => {
        const unremovableRules = getMockRule('test2', '321', true);
        const state = SearchOptionReducer(
            {
                condition: 'and',
                rules: [getMockRule('test1', '123'), unremovableRules],
            },
            { type: SearchOptionsActionsType.REMOVE_ALL }
        );
        expect(state).toEqual({
            condition: 'and',
            rules: [unremovableRules],
        });
    });
});
