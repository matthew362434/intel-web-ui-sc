...
export const getFormattedToHourMinSec = (duration: string): string => {
    if (isFormattedHourMinSec(duration)) {
        return duration;
    }

    const numDuration = Number(duration);
    return Number.isNaN(numDuration) ? '' : formatToHourMinSec(numDuration);
};

export const removeProgressDecimals = (progress: string): string => {
    const [val] = progress.replace('%', '').split('.');
    return val ? `${val}%` : '';
};
