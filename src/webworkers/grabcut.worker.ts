...
export class Grabcut {
    ...
    runGrabcutRect({ roiRect, resizedImg, scale }: GrabcutDependencies): Point[] {
        ...
        try {
            return this.extractPolygons(resizedMask, roiRect);
        } finally {
            this.mask.delete();
    ...
    extractPolygons(src: OpenCVTypes.Mat, roiRect: OpenCVTypes.Rect): Point[] {
        let thresholdMask;
        let contours;
        let contour;
        let points = [];
        ...
        try {
            ...
            this.removeBorderContour(thresholdMask);
    ...
    removeBorderContour(src: OpenCVTypes.Mat): void {
...
export const WorkerApi = { Grabcut, isModuleLoaded, terminate };
