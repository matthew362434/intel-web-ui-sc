...
// Request from within the webworker must be done using self.importScripts.
// This will let the service worker take over, saving and serving it later from cache
...
export class IntelligentScissors {
    img: OpenCVTypes.Mat;
...
export const isModuleLoaded = async (moduleName: string): Promise<boolean> => {
    const loadedCV = await cv;
    return !!loadedCV[moduleName];
};

export const optimizePolygon = (prevPolygon: Polygon): Polygon => {
    return { ...prevPolygon, points: optimizePolygonAndCV(CV, prevPolygon.points) };
};

export const optimizeSegments = (segments: Point[][]): Polygon => {
...
export const WorkerApi = { IntelligentScissors, isModuleLoaded, optimizePolygon, optimizeSegments, terminate };
